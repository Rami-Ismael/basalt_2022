...
from basalt_2022_behavioural_cloning_baseline.openai_vpt.agent import PI_HEAD_KWARGS, MineRLAgent
from torch.utils.data import DataLoader
from basalt_2022_behavioural_cloning_baseline.openai_vpt.lib.tree_util import tree_map
...
BATCH_SIZE = 52 if USING_FULL_DATASET else 16
# Ideally more than batch size to create
# variation in datasets (otherwise, you will
# get a bunch of consecutive samples)
# Decrease this (and batch_size) if you run out of memory
N_WORKERS = 64 if USING_FULL_DATASET else 20
...
def behavioural_cloning_train(data_dir, in_model, in_weights, out_weights):
    ...
    bkup_out_path = out_weights[:-8]
    bkup_out_path = bkup_out_path + "_bkup.weights"
    ...
    for batch_i, (batch_images, batch_actions, batch_episode_id) in enumerate(data_loader):
        ...
        if batch_i % SAVE_BATCHES == 0:
            ...
            time_since_start = time.time() - start_time
            print(f"Time: {time_since_start:.2f}, Batches: {batch_i}, Saved Current state to: {tmp_out_path}")

        if batch_i % BCKUP_BATCHES == 0:
            if batch_i != 0:
                state_dict = policy.state_dict()
                torch.save(state_dict, bkup_out_path)
                print("Backed up.")

    state_dict = policy.state_dict()
    torch.save(state_dict, out_weights)
    time_since_start = time.time() - start_time
    print(f"Time: {time_since_start:.2f}, Batches: {batch_i}, Saved To: {out_weights}")
